...
Edge.prototype = {
	validate: function() {
		if(this.v1 === this.v2) throw new Error('wtf?');
		if(this.v1.id < this.v2.id) {
			this.validId = this.v1.id + ':' + this.v2.id;
		} else {
			this.validId = this.v2.id + ':' + this.v1.id;
		}
	},
	getLength: function() {
	...
	removeItselfFromVertices: function() {
